	...
	struct convert<com::robotraconteur::imaging::camerainfo::CameraCalibrationPtr>{
		...
		static bool decode(const Node& node, com::robotraconteur::imaging::camerainfo::CameraCalibrationPtr& rhs){
			...
			if(node["distortion_info"]){
				...
				if(type=="double[]"){
					RRArrayPtr<double> my_array = AllocateEmptyRRArray<double>(node["distortion_info"]["value"].size());
					for (int i = 0; i < node["distortion_info"]["value"].size(); i++) {
						my_array->at(i) = node["distortion_info"]["value"][i].as<double>();
					}
					varval=my_array;
				}
				if(type=="int32[]"){
					RR::RRArrayPtr<int> my_array = AllocateEmptyRRArray<int>(node["distortion_info"]["value"].size());
					for (int i = 0; i < node["distortion_info"]["value"].size(); i++) {
						my_array->at(i) = node["distortion_info"]["value"][i].as<int>();
					}
					varval=my_array;
				}
				if(type=="uint32[]"){
					RR::RRArrayPtr<uint32_t> my_array = AllocateEmptyRRArray<uint32_t>(node["distortion_info"]["value"].size());
					for (int i = 0; i < node["distortion_info"]["value"].size(); i++) {
						my_array->at(i) = node["distortion_info"]["value"][i].as<uint32_t>();
					}
					varval=my_array;
				}
